...
/// attribute correctly (RT-W3). Factored so tests cannot drift from `serve_listener`.
...
///
/// Spawns the change detector and wires graceful shutdown. Tests bind
/// `127.0.0.1:0`, read `local_addr`, then call this so DoD-6 exercises
/// production shutdown wiring (not a fork).
