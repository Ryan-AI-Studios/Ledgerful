...
pub fn execute_hook_post_commit_for_layout(layout: &crate::state::layout::Layout) -> Result<()> {
    ...
    let _handle = thread::Builder::new()
        .name("hotspot-trend-recorder".to_string())
        .spawn(move || {
            let start = std::time::Instant::now();
            let result = record_hotspot_trends(&repo_root, &db_path);
            if let Err(ref e) = result {
                tracing::debug!("Post-commit hook: hotspot trend recording failed: {}", e);
            }
            let elapsed = start.elapsed();
            if elapsed > Duration::from_secs(5) {
                tracing::debug!(
                    "Post-commit hook: hotspot trend recording took {:?}, exceeding 5s budget",
                    elapsed
                );
            }
            let _ = done_tx.send(());
    ...
    // Wait up to the 5-second budget for the recorder to finish. On Windows
    ...
    if done_rx.recv_timeout(Duration::from_secs(5)).is_err() {
        tracing::debug!(
            "Post-commit hook: hotspot trend recording did not complete within 5s budget"
        );
    }
