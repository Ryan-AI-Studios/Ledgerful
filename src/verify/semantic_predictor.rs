...
mod tests {
    ...
    #[test]
    fn query_similar_outcomes_skips_tcp_closed_embed_url_not_completions_base() {
        let conn = setup_db();
        let server = MockServer::start();
    ...
    #[test]
    fn query_similar_outcomes_http_dead_respects_short_timeout() {
        let conn = setup_db();
        let server = MockServer::start();
        server.mock(|when, then| {
            when.method(httpmock::Method::POST).path("/v1/embeddings");
        ...
        let start = std::time::Instant::now();
        let result = query_similar_outcomes(&conn, &config, "similar change", 30);
        assert!(
