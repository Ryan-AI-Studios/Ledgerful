...
fn test_team_secret_to_bundle_key_is_deterministic() {
    ...
    let key1 = crypto::derive_bundle_key(secret, salt);
    let key2 = crypto::derive_bundle_key(secret, salt);
...
fn test_bundle_key_varies_with_salt() {
    ...
    let key1 = crypto::derive_bundle_key(secret, salt1);
    let key2 = crypto::derive_bundle_key(secret, salt2);
...
fn test_aead_round_trip() {
    ...
    let (ciphertext, tag) = crypto::seal(plaintext, &key, &nonce).expect("Encryption failed");
    let decrypted = crypto::open(&ciphertext, &tag, &key, &nonce).expect("Decryption failed");
...
fn test_aead_open_rejects_tampered_ciphertext() {
    ...
    let (mut ciphertext, tag) = crypto::seal(plaintext, &key, &nonce).expect("Encryption failed");

    // Tamper with ciphertext
    ciphertext[0] ^= 0xFF;

    let result = crypto::open(&ciphertext, &tag, &key, &nonce);
    assert!(
        result.is_err(),
