...
| `review --json` | yes (0304) | yes | schemaVersion 1 object `kind: "review"`; `analysisMode: "range"`. Compose `base…head` files + in-memory impact counts + bounded ledger search. Does **not** rewrite `latest-impact.json`. Human path is 2–4 lines. CLI-only. Not `scan --pr`. Not change-context |
