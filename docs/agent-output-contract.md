...
| `viz` / `viz --view services` | no | yes (path + `#evidence` tokens) | No `--json`. Default dest `reports/graph.html` / `reports/services.html`. Offline vis-network 10.1.2. Graph: `source: Cozo nodes/edges` then `Drawn: N nodes, M edges`. Services gated: `source: declared overlay; inference gated`. Services stdout has no `Drawn:`. Machine=false. |
