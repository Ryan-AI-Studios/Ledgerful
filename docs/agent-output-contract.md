...
| `services list --json` / `services diff --json` | yes | yes | schemaVersion 1 object, collection `results`. Gated empty keeps `emptyReason`/`message`. Always `inferenceState`. Omit-empty `declared`. `--preview` **or gated persist-empty fill** adds `preview: true` and never writes `sessionNotices`. Kind-token service names (`BUILDER`/`TEST`/`METHOD_CALL`/`DECORATOR`/`APP_METHOD`) omitted unless declared. Additive `sessionNotices` (0300; skip empty) on persist-empty only (not on fill). Item additive `source` / omit-empty `root` / `--full` `files` + `filesTruncated` |
