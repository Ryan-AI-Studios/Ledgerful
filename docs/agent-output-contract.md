...
|---|---|---|---|
| `doctor --json` | yes | yes | schemaVersion 1 findings; additive `environment.githubLatest` (0205); additive per-finding `sessionPriority` `now`\|`later` (0225 emission-only; **0295** hygiene/`is_hygiene` is `later` on observe and enforce; 0225 signing trio later-on-observe only); additive per-finding `acknowledged` / `acknowledgedAt` (0226); additive top-level `completionReadiness` (0311: `cold`\|`loading`\|`busy`\|`ready`\|`unreachable`\|`fallback_failed`; omit when generation is not configured). schemaVersion stays 1. Sidecar `doctor-results.json` does **not** include `githubLatest`, `sessionPriority`, `acknowledged`, `acknowledgedAt`, or `completionReadiness`. Optional embed miss is `embed-unreachable` (optional warn), not `ERROR ledgerful::embed::client` (0285). |
