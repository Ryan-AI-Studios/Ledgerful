...
| `doctor.topFindings` | array | From sidecar `findings` after a successful `doctor` write (0129): block+warn only (**no category filter** — optional-category warns included), severity-first (block before warn), then code/message, **cap ≤5**. Each entry: `code`, `severity`, `message`, optional `remediation` when present (never `null`). Empty when doctor not run / sidecar missing / only info findings / pre-0129 count-only sidecar. Full finding list remains on `ledgerful doctor --json`. |
