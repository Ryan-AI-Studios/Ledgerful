...
**0417** (`ledger recovery plan --json` is schemaVersion 1 `kind: "ledgerAdoptionPlan"`; `ledger recovery apply --json` is `kind: "ledgerAdoptionResult"`; `verify --signatures --chain --accept-adoption` adds an `adoption` object and keeps schemaVersion 1).
...
| `change-context --json` | yes | yes | impact-shaped packet. Additive omit-empty `completeness` (0308/0347). `--timeout` is an **overall analysis** budget (`hotspots` list/explain `--timeout` is overall emit, 0349; unscoped `audit` `--timeout` is overall emit, 0350); working-tree and prospective default 25s when omitted; `0` disables (unbounded, may hang). Default Instant skips the unindexed `symbols` complexity fallback. Token `prospective analysis stopped: overall budget` is stderr-only |
...
| `ledger graph --json` | yes | no (truncate on stderr when completeness present) | Object `{exact, derived, heuristic}` (no `schemaVersion`). Additive omit-empty `completeness` `{stop: "cap", maxDepth, maxNodes}` only when assemble hid a further hop or node. `--compact` refused with `--json`. Truncation warning is stderr. Human assemble-empty buckets name the reason and `ledgerful ledger audit <entity>` (0426); `--layer` hide stays `None.`. JSON keys stay the three arrays + omit-empty `completeness` (no `entity` / `emptyReason` / `next`). |
...
| `viz` / `viz --view services` | no | yes (path + `#evidence` tokens) | No `--json`. Default dest `reports/graph.html` / `reports/services.html`. Offline vis-network 10.1.2. Graph **stdout**: `source: Cozo nodes/edges` then `Drawn: N nodes, M edges`. HTML `#evidence` / `graph_banner_tokens` stay tokens-only (no `Drawn:`). Services gated: `source: declared overlay; inference gated`. Services stdout has no `Drawn:`. Machine=false. |
...
| `dependencies list --json` | yes (0153) | yes | schemaVersion **1** envelope; `mode`: `direct` (default) \| `all`; live Cargo.toml+lock — not Cozo; additive omit-empty `packages[].relation` `root`\|`direct`\|`transitive` on `--all` only (0427); see schema below |
| `dependencies audit --json` | yes | yes | Unwrapped `OsvResult` (`results[]` at top level). **No** `schemaVersion` wrap. Human path always prints interpolated `Source: --input {input}` + `Findings: N`; `Next:` official `osv-scanner scan --format json -L Cargo.lock > {input}` only when N=0. Does not scan. `populate_kg` stays. |
| `impact --json` | yes | yes | impact packet (`schemaVersion` string `"v1"`). `--timeout` is an overall analysis budget (0347; `hotspots` list/explain `--timeout` is overall emit, 0349; unscoped `audit` `--timeout` is overall emit, 0350). Working-tree and prospective `--paths` default 25s when omitted; `0` disables (unbounded, may hang). Default Instant skips the unindexed `symbols` complexity fallback (`hotspots[].complexity` stays 0 for files absent from `project_symbols`; score is still `f_norm * c_norm`). Additive omit-empty `completeness` with `scope: "overall"` when the overall analysis **stops** (budget Instant **or** cancel), including working-tree without `--timeout` (0374). A history-only `completeness` (`scope` omitted) may appear when `capped_by_overall` mins the 45s history walk to 25s. Token `prospective analysis stopped: overall budget` is stderr-only |
| `scan --impact --json` | yes | yes | impact packet (`schemaVersion` string `"v1"`; no top-level `kind`). `--timeout` requires `--impact` and is an overall analysis budget (0347; `hotspots` list/explain `--timeout` is overall emit, 0349; unscoped `audit` `--timeout` is overall emit, 0350). Working-tree, `--base-ref`, and prospective `--paths` default 25s when omitted; `0` disables (unbounded, may hang). Working-tree / `--base-ref` always take the persist-aware arm (0424) — omitted `--timeout` is the same path as `--timeout 25` (`should_skip_persist` / 0374 still applies). `--timeout 0` stays unbounded and may persist unless cancel. That unified arm applies `LEDGERFUL_HISTORY_BUDGET_SECS` / `[hotspots] history_budget_secs` (was silent `load_config` only on omitted `--timeout`). Default Instant skips the unindexed `symbols` complexity fallback (`hotspots[].complexity` stays 0 for files absent from `project_symbols`; score is still `f_norm * c_norm`). Additive omit-empty `completeness` with `scope: "overall"` when the overall analysis **stops** (budget Instant **or** cancel), including working-tree without `--timeout` (0374). A history-only `completeness` (`scope` omitted) may appear when `capped_by_overall` mins the 45s history walk to 25s. Token `prospective analysis stopped: overall budget` is stderr-only |
...
| `services list --json` / `services diff --json` | yes | yes | schemaVersion 1 object, collection `results`. Gated empty keeps `emptyReason`/`message`. Always `inferenceState`. Omit-empty `declared`. `--preview` **or gated persist-empty fill** adds `preview: true` and never writes `sessionNotices`. Kind-token service names (`BUILDER`/`TEST`/`METHOD_CALL`/`DECORATOR`/`APP_METHOD`) omitted unless declared. Zero-`api_routes` inferred names omitted unless `--include modules` (omit-empty `includeModules: true`). Additive `sessionNotices` (0300; skip empty) on persist-empty only (not on fill). Item additive `source` / omit-empty `root` / `--full` `files` + `filesTruncated` |
| `deploy impact --json` | yes | yes | schemaVersion 1 object, collection `results`. Cheap git-diff classify (no SQLite / orchestrator). Always `defaultPatterns` / `classifiers` (`Unknown` is enum-parity; Helm/CiWorkflow are not default globs). Gated empty is **only** `coverage.enabled && !coverage.deploy.enabled` (explicit deploy-section disable) and keeps `emptyReason`/`message`. Product-default / global-off classifies; empty then is `noMatches`. Disabled `message` must **not** start with “No deployment impact detected.” (that sentence is `noMatches` only). Overall-stop empty omits `emptyReason`/`message` and adds omit-empty `completeness` (`stop` `budget`\|`cancelled`, `scope: "overall"`, `stage: "deploy"`, `budgetSecs`). Item omit-empty `coupledFiles` / `highBlastResources`. `--timeout` on `impact` (not parent `deploy`) is overall emit (default 25s; `0` unlimited; env `LEDGERFUL_DEPLOY_OVERALL_BUDGET_SECS` / `[coverage.deploy] overall_budget_secs`). Token `deploy impact stopped: overall budget` is stderr-only on Budget stop; cancel is silent. Additive `sessionNotices` (0300; skip empty) is `coverage.deploy` only on gated empty — never `coverage.global` on this command. |
| `observability coverage --json` | yes | yes | schemaVersion 1 object, collection `results`. Always `inputs` (`directory` / `ingest` / sorted `kinds`) + `notWired: ["endpoints"]`. Empty keeps `emptyReason`/`message`. Item always `health` (`covered`\|`missing`) plus snake `slo_count` / `metric_count`. `preview: true` means rows are disk-derived (not graph-persisted): `--preview` **or persist-empty fill** when repo-root YAML yields a `kind: Service`. **Only `--preview` never opens Cozo**; fill may open Cozo to learn persist is empty, then emit disk rows without writing. Fill never writes `sessionNotices`. Persist empty (fill miss) keeps 0215 taxonomy + additive `sessionNotices` (0300; skip empty). Omit-empty `parseErrors[]` `{path, reason}` |
| `observability diff --json` | yes | yes | schemaVersion 1 object `kind: "observabilityDiff"`. Collection `changed`. Always `unchanged_count` / `indexedCount` / `resultCount` (`resultCount` = `changed.len()`). Empty keeps `emptyReason`/`message`. Item omit-empty `sourceFile`. `preview: true` means rows are disk-derived: `--preview` **or persist-empty fill** when disk YAML yields SLO/metric/alert/signal nodes (`indexedCount` is that disk node count). **Only `--preview` never opens Cozo**. Persist empty (fill miss) keeps 0215; preview/fill empty does **not** tell the user to `index --analyze-graph`. Omit-empty `parseErrors[]` |
...
| `sync verify --json` | yes (0365; 0401; `feature = "sync"`) | yes* | schemaVersion 1; **no** `kind`. Always `ok` (bool) + `verdict` (`ok`\|`missingFile`\|`missingSecret`\|`decryptFailed`\|`unknownDevice`\|`signatureFailed`\|`integrityFailed`\|`schemaInvalid`). **`ok` and process exit must agree** (`ok` → 0; every non-`ok` → 1 with JSON then `request_exit(1)`). Success always `version`/`deviceId`/`bundleHlc`/`entryCount`. Failure omit-empty `message` (`unknownDevice` names the id; `missingSecret` names `LEDGERFUL_SYNC_SECRET`). JSON `message` is the raw string (not `{verdict}:`-prefixed). Human failures print `{verdict}: {message}` on **stderr**, empty stdout, exit 1. `--json` stderr may also name the token. |
...
string `"already_shown"`. `deploy impact` empty may emit `coverage.deploy` only
(explicit `coverage.enabled && !coverage.deploy.enabled`); product-default /
global-off classify/`noMatches` never writes a deploy-impact notice.
Item `next` / empty `message` stay. Human collapse is
...
| `ask` `[Evidence]` line (0312 / 0395) | **stderr** | Human-only. Four tokens `semantic=N bm25=N kg=N snippets=N`. `bm25` is the pre-fusion Tantivy kept-list size (always-blend; nonzero when FTS ran even if vectors were nonempty). `semantic` excludes `source` starting with `Knowledge Graph`. Omit-empty ` structural=N` when the symbol/`search_term_exact` list contributed (including CamelCase / PascalCase TermQuery hits after `code_tokenizer_v3`). `snippets` is the **post-fusion** truncated chunk count (sum of origin counters may exceed it; per-list RRF once-per-path may change who lands in the truncated top-N). Skip path (LLM-instruction / ping on the global arm) emits zeros. No `ask --json`. |
...
| `ask` `[AskMeta]` trailer (0430) | **stdout** | Human+agent. After a model answer: `[AskMeta] semantic=N bm25=N kg=N snippets=N gatherMs=N provider=P truncated=yes\|no`. Omit-empty ` structural=N` after `snippets=`. `provider` is CLI kebab (`local` / `gemini` / `ollama-cloud` / `openrouter`). `truncated=yes` only for `is_length_stop`. Skip-then-answer emits zeros. No trailer on refuse / empty-snippets / degrade-to-context. No `ask --json`. |
...
| `packages[].relation` | **0427.** Omit-empty. Present only on `--all`: `root` \| `direct` \| `transitive` from the H1 `(name, locked version)` map (not name-only). Direct mode omits the key. |
...
| `observability coverage --json` | `results` | Item `slo_count` / `metric_count` stay snake. Always `inputs` + `notWired: ["endpoints"]` (lowercase command-mode token). Item always `health`. `preview: true` = disk-derived rows (`--preview` or persist-empty fill). **Only `--preview` never opens Cozo.** Fill skips 0215 / `sessionNotices`. Omit-empty `parseErrors[]`. |
| `observability diff --json` | `changed` | `kind: "observabilityDiff"`. Always `unchanged_count` / `indexedCount` / `resultCount`. Item omit-empty `sourceFile`. `preview: true` = disk-derived rows (`--preview` or persist-empty fill); fill `indexedCount` is disk node count. **Only `--preview` never opens Cozo.** Omit-empty `parseErrors[]`. |
| `data-models list --json` | `models` | Item `file_path` stays snake. Always `includeFixtures` (bool) + `fixturesOmitted` (omitted `(name, language, kind, file_id)` identity count; 0 when included or none omitted). Always sorted `supportedExtractors` (`goJsonTaggedStruct` / `pythonModelPath` / `rustPersistenceDerive` / `typescriptEntityOrModelDir`) and `notWired` (`cppWalker` / `javascript` / `sqlMigrations`). Token `rustPersistenceDerive` covers FromRow/Queryable/Insertable **and** rusqlite `Row` mapper functions / `query_map` struct construction (not a derive-only claim). Omit-empty `next` (`ledgerful data-models list --include-fixtures`) only when `!includeFixtures && fixturesOmitted > 0` and the collection is empty. Default product inventory omits `is_test_path`. `--include-fixtures` restores. Item `fieldImpact: "unsupported"` on each `models[]` object; **absent** when the array is empty (no envelope `fieldImpact`). Post-omit product-empty is `emptyReason: noMatches` (not `noIndexedData`); message names persistence derives and rusqlite Row mappers and says SQL migrations are not extracted. Truly empty catalog stays the 0207 list envelope (no `emptyReason`). Human truly-empty prints a second extractor line. No `fields` / `table` / `evidence` keys. No MCP tool. |
...
| `services list --json` / `services diff --json` (alias) | `results` | Gated empty keeps `emptyReason: "disabledByConfig"` + `message`. `list` is primary. Always `inferenceState` (`disabledGlobally` \| `disabledForServices` \| `enabled`). Omit-empty `declared[]` `{name, root}`. `--preview` **or gated persist-empty fill** sets `preview: true` (in-memory infer; does not persist `service_name` and does not write the 0300 cookie). Enabled persist-empty does **not** fill. Kind-token names (`BUILDER`/`TEST`/`METHOD_CALL`/`DECORATOR`/`APP_METHOD`) omitted unless declared. Zero-`api_routes` inferred names omitted unless `--include modules`; omit-empty envelope `includeModules: true` only when that flag is set. Item `source`: `declared` if the name is in `[services]`, else `preview` when `preview: true`, else `inferred`. Omit-empty `root` (config root or preview directory; omit on persisted inferred). `--full` omit-empty `files` (cap 200) + `filesTruncated`. |
...
| `query` | string | Joined query string (`query.join(" ")`). Unquoted multi-word OK like `search`; quoted one-token is unchanged |
...
- **Write policy:** prospective does **not** rewrite `latest-impact.json`
  (in-memory only). Working-tree / `--base-ref` without `--paths` always
  use the persist-aware arm (0424), including omitted `--timeout`. Durable
  write is skipped when the overall analysis stops (`completeness.scope=overall`
  from a budget Instant **or** the cancel flag). `--timeout 0` is unbounded
  and may persist unless cancel — no truncated persist (0374). History env
  `LEDGERFUL_HISTORY_BUDGET_SECS` now applies on that omitted-`--timeout` path.
- **Overall deadline (0347 / 0419):** cooperative provider-boundary Instant
  (default 25s on prospective, working-tree, and `--base-ref`; CLI `--timeout`
  / env `LEDGERFUL_PROSPECTIVE_BUDGET_SECS` / `[impact]
  prospective_budget_secs`). `0` disables the clock (unbounded; may hang).
  Instant present skips the unindexed `symbols` complexity fallback. Not a
  thread kill. Scoring runs **once** after the enrichment loop or stop
  (`f_norm * c_norm` unchanged). Distinct from history-walk `[hotspots]
  history_budget_secs` (default 45; `capped_by_overall` may emit a
  history-only `completeness` with `scope` omitted). Review `--timeout` is an overall emit budget (0348).
