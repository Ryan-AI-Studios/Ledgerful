...
| `change-context --json` | yes | yes | impact-shaped packet. Additive omit-empty `completeness` (0308/0347). `--timeout` is an **overall analysis** budget (not the history-walk `--timeout` on `hotspots`/`audit`); omitted on working-tree; prospective default 25s. Token `prospective analysis stopped: overall budget` is stderr-only |
...
- **Overall deadline (0347):** cooperative provider-boundary Instant (default
  ...
  **once** after the enrichment loop or stop. Distinct from
  `hotspots`/`audit` history-walk `--timeout`. Review `--timeout` is an overall emit budget (0348).
