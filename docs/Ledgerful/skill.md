...
- If a command reports that the index is `[STALE]`, append `--auto-index` to commands that support
  it: **`search`, `ask`, `hotspots`, `dead-code`, `verify`** (prefer passing it proactively when
  unsure). Time-stale **and** content-hash drift-stale both trigger refresh; never-indexed runs a
  full bootstrap. Never idle SCIP.
