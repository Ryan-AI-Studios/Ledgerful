...
`ledgerful verify --dry-run` without `--scope` previews the pre-push **fast** plan (executed `verify` without flags stays **full**; `--scope full --dry-run` is the full preview). `--json --dry-run` / `--json --health` / `--json --signatures` emit sibling `kind`s (`verifyDryRun` / `verifyHealth` / `verifySignatures`); executed `verify --json` still has no `kind`. Mixed diagnostic flags refuse (empty stdout). Fully-clean auto-policy dry-run skips prediction.
