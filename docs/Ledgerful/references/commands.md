...
`plan --output PATH` is read-only. It writes `kind: "ledgerRecoveryManifest"` (`manifestVersion` 1) with `create_new` and refuses a path under `.ledgerful`. `repoIdentity` is `git remote get-url origin`. The digest covers that URL, the stored head, and the sorted entry hashes. It is not the 0416 `sourceDigest`. `apply --manifest PATH` checks that file against a fresh read and does not mutate. `apply --manifest PATH --yes` backs up the database (`integrity_check` and `foreign_key_check`), then appends one signed maintenance row. Existing `prev_hash` values stay. Repeat apply returns `alreadyApplied`. `--json` envelopes are schemaVersion 1 (`ledgerAdoptionPlan` / `ledgerAdoptionResult`) and include `historicalContinuity: notEstablished`. `verify --signatures --chain` still fails on extra genesis. `--accept-adoption` requires `--chain`.

## `ledgerful ledger diagnose`

Read-only chain diagnosis. Exit 0 when the database was read, including when anomalies exist. Exit 1 when the database is missing. `--json` is schemaVersion 1 `kind: "ledgerChainDiagnosis"` (camelCase anomaly rows, exact totals, paged `anomalies`). `--limit` defaults to 100 and must be 1 through 1000. `--output` writes a new file and refuses an existing path or anything under `.ledgerful`. `--manifest --output` writes unsigned `kind: "ledgerRecoveryManifestProposal"` (`candidateTxIds` are not approval to mutate). Does not print private keys, raw signatures, or raw public keys. `verify --chain` still exits non-zero and still caps `breaks` at 20.

`ledger re-sign` checks topology before key access and before backup. Extra genesis or forks print `BLOCKED`, exit 1, and do not create `ledger.db.*.bak`. A bad signature can look like an orphan; diagnose still reports that class, and re-sign still repairs it. A clear chain still previews with `--dry-run` (exit 0) and mutates only with `--yes`.

## `ledgerful ledger gc --dry-run`

Human preview only (`Gc` is not machine; no `--json`). `--dry-run` with neither `--stale` nor `--orphans` is a 0-plan (`No selector specified…` + the dry-run footer); it is not Usage and does not scan layout, sidecars, or TTL. Prints a labeled count for every requested class (`--stale` and/or `--orphans`) even when the first class is empty. Both flags use the same TTL PENDING selector (`started_at` older than `--ttl-hours`, default 72; days via `div_ceil(24)`). `--orphans` is **not** a git-commit scan. Protected sidecars (0074 `promote_failed` / `HEAD-matching orphan`) are listed, not candidates. Footer `Dry-run completed. No transactions were modified.` is unconditional. Dry-run always exits 0; write-path refuse `Err` is unchanged. Combined `--force` unions+dedupes before rollback. Never run without `--dry-run` on an operator repo.
...
`--timeout` is the overall list/explain emit budget (default 25s; `0` disables). Place `--timeout` / `--commits` **before** `explain`. History stays `[hotspots] history_budget_secs`. `hotspots explain --json` is `kind: hotspotExplanation`. Additive omit-empty `contributingCommits` (cap 5) or `emptyReason: notInWindow` on a complete window that does not touch the file. `hotspots trend` / `budget` ignore parent `--timeout`.
...
Default list is live `Cargo.toml` + lock (Kind + Locked). `--all` is the lock dump with `relation` `root` \| `direct` \| `transitive` from the root package's locked versions (not name-only). Footer `Direct: N · Transitive (lock-only): M` counts lock rows. `audit --input` imports osv-scanner JSON into the graph — it does not scan. Empty human names the file and official `osv-scanner scan --format json -L Cargo.lock > {input}` next. `--json` stays the unwrapped `OsvResult`.

## `ledgerful services list`

Index topology inventory. Alias: `diff`. Persist assignment still follows `coverage.enabled`. When persist rows are empty **and** inference is gated, default `list` fills from the same non-writing infer as `--preview` (`preview: true`; human title `Service topology preview (not persisted)`). `--preview` infers from the current index without persisting `service_name` or flipping coverage. Kind-token names (`BUILDER`, `TEST`, `METHOD_CALL`, `DECORATOR`, `APP_METHOD`) are omitted unless declared under `[services]`. Zero-`api_routes` inferred directory names are omitted unless `--include modules` (human footer `Modules omitted: N …` when any row is hidden). `--full` lists up to 200 files per service. Gated truly-empty names declared `[services]` when present. Keep `hide` on root `--help` (0289). `--json`: schemaVersion 1 object, collection `results`; gated empty keeps `emptyReason: "disabledByConfig"`; additive `inferenceState` / omit-empty `declared` / omit-empty `includeModules` / `preview: true` on `--preview` or gated fill / item `source`. Session-once (0300): persist-empty only; `--preview` and gated fill never write the cookie. JSON `message` stays and may add `sessionNotices`. Honor `already_shown` — do not re-ask HITL.

## `ledgerful deploy impact`

Working-tree git-diff classify of deployment manifests (no impact orchestrator, no SQLite). Product-default / global-off still classifies. Gated empty only when `coverage.enabled && !coverage.deploy.enabled` (explicit deploy-section disable). Disabled copy names the switch and does **not** start with “No deployment impact detected.” Human `noMatches` leads with the classifier sentence, then that verdict, then one next (add a matching manifest — not `index --incremental`). JSON `emptyReason` / `message` stay; no JSON `next`. Keep `hide` on root `--help` (0289). `--timeout` is on **`impact`**, not parent `deploy` (`ledgerful deploy --timeout 5` is a clap error). Default overall emit budget is 25s (`0` disables; env `LEDGERFUL_DEPLOY_OVERALL_BUDGET_SECS` / `[coverage.deploy] overall_budget_secs`). `--json`: schemaVersion 1 object, collection `results`; always `defaultPatterns` / `classifiers`; gated empty keeps `emptyReason`/`message`; overall-stop completeness uses stage `deploy` and omits `emptyReason`. Stderr `deploy impact stopped: overall budget` is Budget-only (cancel is silent). Default globs: Dockerfile, docker-compose YAML, `*.tf`, `k8s/**/*.yaml`. Helm / CiWorkflow classify only with a user glob; `Unknown` is enum-parity only. Session-once (0300): gated empty (explicit deploy-off) may add `sessionNotices.coverage.deploy` only — never `coverage.global` on this command. Second human empty contains `Already shown this session.` and omits `config set`. Honor `already_shown` — do not re-ask HITL.

## `ledgerful observability coverage`

Empty OpenSLO inventory (any empty reason). Disk fill (0400): when persist has no service rows, default coverage emits the same disk parse as `--preview` (`preview: true`) if repo-root `observability/` has a `kind: Service`. `[services]` overlay is persist/ingest only (after `index --analyze-graph`), not the disk fill. `--preview` parses that directory without opening Cozo or writing state. A SLO-only DX1 template stays empty on persist coverage. Coverage stays opt-in (`coverage.enabled` default false). Keep `hide` on root `--help` (0289). `--json`: schemaVersion 1 object, collection `results`; always `inputs` / `notWired: ["endpoints"]`; empty keeps `emptyReason`/`message`. Persist session-once (0300): remaining persist-empty only (fill never writes the cookie). `--preview` never writes the cookie. Honor `already_shown` — do not re-ask HITL.

## `ledgerful observability diff`

Working-tree OpenSLO change list (0146 git-status path match on `sourceFile`). Persist reads graph nodes (`slo` / `metric` / `alert` / `observability_signal`). Persist-empty fills from disk YAML when those kinds exist (`preview: true`; `indexedCount` is the disk node count). `--preview` parses disk YAML without Cozo. Keep `hide` on root `--help` (0289). `--json`: schemaVersion 1 object `kind: observabilityDiff`, collection `changed`; always `unchanged_count` / `indexedCount` / `resultCount`. Item omit-empty `sourceFile`. Persist empty (fill miss) keeps 0215 taxonomy; preview/fill empty does not recommend `index --analyze-graph`.
