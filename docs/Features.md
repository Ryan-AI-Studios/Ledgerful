...
*   **Team Sync [Experimental]**: Opt-in encrypted ledger entry bundles between developer devices (`ledgerful sync`). Default feature surface; runtime `[sync].enabled = false` forever until you opt in. Pair accept and multi-device polish land in later tracks. See `docs/team-sync.md`.
