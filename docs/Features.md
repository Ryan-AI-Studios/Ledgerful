...
*   **Team Sync [Available — opt-in shared-folder v1]**: Opt-in encrypted ledger entry bundles between developer devices (`ledgerful sync`). Default feature surface; runtime `[sync].enabled = false` forever until you opt in. Pairing (0111), secure transport/apply (0112), and setup checklist / status next-action / gated enable (0113) are real. `sync cursor` explains Hybrid Logical Clocks without inventing wall-clock lag; `sync::run` appends JSON-line events to `sync.log`; `sync verify --json` emits a closed verdict. Not default-on, not cloud SaaS, not CRDT. See `docs/team-sync.md`.
