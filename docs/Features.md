...
*   **Team Sync [Experimental]**: Opt-in encrypted ledger entry bundles between developer devices (`ledgerful sync`). Default feature surface; runtime `[sync].enabled = false` forever until you opt in. Pairing invite/accept and peer trust are real (0111); multi-device apply polish and Available marketing land in later tracks. See `docs/team-sync.md`.
