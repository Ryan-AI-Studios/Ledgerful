...
*   **Config provenance (0323):** `config verify` attributes file/env/default via `origin`/`location` (no secret values). `config schema` names the source file and distinguishes declared-optional from unknown requiredness. Empty `.env.example` `KEY=` stays not required (0216). Human `config diff` prints reference paths. `config view --section`/`--key` stays a redacted Config dump.
