...
*   **Config provenance (0323 / 0342):** `config verify` attributes file/env/default via `origin`/`location` (no secret values). Empty/whitespace TOML `base_url` origin agrees with `resolve_string` (no trim): whitespace is file, empty after env/dotenv miss is default and is omitted without `--verbose`. `config schema` names the source file and distinguishes declared-optional from unknown requiredness. Empty `.env.example` `KEY=` stays not required (0216). Human `config diff` prints reference paths. `config view --section`/`--key` stays a redacted Config dump.
