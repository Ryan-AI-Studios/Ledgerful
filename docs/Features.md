...
*   **Service topology inspectability (0356 / 0397)**: persist inventory still follows `coverage.enabled` (not a working-tree diff). On a gated persist-empty tree, default `services list` fills from a non-writing preview (`preview: true`; human title `Service topology preview (not persisted)`). `--preview` remains and never writes `service_name` or coverage. Kind-token names (`BUILDER` / `METHOD_CALL` / `TEST` / `DECORATOR` / `APP_METHOD`) are omitted unless declared. `--full` lists up to 200 files per service. Surfaces stay persist-`service_name` readiness. Gated truly-empty names declared `[services]` and does not print a header-only table.
