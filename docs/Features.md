...
*   **Garbage Collection**: Identify and prune TTL PENDING transactions via `ledger gc --stale` / `--orphans` (same selector). Promote-failed and HEAD-matching hook sidecars are GC-ineligible; use `recover-orphan` instead of silent delete.
...
*   **Security boundaries operator envelope (0359)**: `security boundaries` leads with `@id` → route, source file, and `enforcement: none` (not a live PDP). `--json` adds `schemaVersion` 1 / `kind: securityBoundaries` / compact `links[]`. Graph URNs stay under `boundaries`; REST `{meta, boundaries}` is unchanged.
*   **Windows-safe human tables (0181)**: Premium human tables (`hotspots`, `timings`, `ledger search`, impact summaries, …) auto-select **ASCII** borders and non-PUA icons when Windows console OutputCP ≠ 65001 (or stdout is non-TTY); UTF-8 rounded borders otherwise. Residual human chrome (doctor marks/bullets/rules/trailers, schema lock cell, security caption arrows/dashes, setup banners, staleness arrow) follows the **same** style. Overrides: `LEDGERFUL_TABLE_STYLE=ascii|utf8|auto`, `LEDGERFUL_TABLE_ASCII=1`, `LEDGERFUL_TABLE_UTF8=1` (`STYLE` wins over simple flags; if both simple flags set, ASCII wins). `NO_COLOR` is color-only and does not force table ASCII. JSON/machine paths stay UTF-8 (data is not ASCII-substituted).
*   **Ledger Federation**: Securely export and sync ledger entries across sibling repositories for cross-repo provenance. **Not** the same as Team Sync (`ledgerful sync`). When two Live cache rows share a path, presented `lastScanned` is the later *parsed* RFC3339 instant (lexicographic max is not enough when fractional width or offset spelling differs).
*   **Service topology inspectability (0356 / 0397 / 0428)**: persist inventory still follows `coverage.enabled` (not a working-tree diff). On a gated persist-empty tree, default `services list` fills from a non-writing preview (`preview: true`; human title `Service topology preview (not persisted)`). `--preview` remains and never writes `service_name` or coverage. Kind-token names (`BUILDER` / `METHOD_CALL` / `TEST` / `DECORATOR` / `APP_METHOD`) are omitted unless declared. Zero-`api_routes` inferred directory names are omitted unless `--include modules`. `--full` lists up to 200 files per service. Surfaces stay persist-`service_name` readiness. Gated truly-empty names declared `[services]` and does not print a header-only table.
*   **Federation export preview (0355)**: `federate export --dry-run` prints a bounded human summary (no mixed JSON banners). `--json` emits `schemaVersion` 1 `kind: federateExportPreview` with HEAD and omit-empty line; `--limit` (default 200) pages the preview only. `federate export` / `--out` still writes the full `schema.json` 1.1 file. Empty preview next is `ledgerful scan --impact`.
*   **Team Sync [Available — opt-in shared-folder v1]**: Opt-in encrypted ledger entry bundles between developer devices (`ledgerful sync`). Default feature surface; runtime `[sync].enabled = false` forever until you opt in. Pairing (0111), secure transport/apply (0112), and setup checklist / status next-action / gated enable (0113) are real. `sync cursor` explains Hybrid Logical Clocks without inventing wall-clock lag; uninitialized human cursor names `notInitialized` and `Next: ledgerful sync init`. `sync::run` appends JSON-line events to `sync.log`; `sync verify --json` emits a closed verdict; human verify failures prefix that verdict on stderr. Not default-on, not cloud SaaS, not CRDT. See `docs/team-sync.md`.
...
*   **Signature / chain verify exit codes** (`verify --signatures` / `--chain`): **0** all valid · **1** INVALID signature, version reject, entity_normalized mismatch, or chain break · **2** reserved (trusted-key policy) · **3** UNSIGNED under require/strict. Federated rows count as `SKIP (federated)` and are not failures. `--json` emits `kind: verifySignatures` with separate **signatures** (validity + trust), **chain** (breaks / extra-genesis / omit-empty stored `head` `{signatureValid, hashMatch, lengthMatch}`), and **checkpoint** (only with `--against-export`) dimensions — extra-genesis is a chain fact, not invalid crypto. `--json --signatures --chain` with only unsigned-required failures stays exit **3** (`ok: false`) when the chain is structurally intact (`breakCount` 0, stored `head` all-true if present, checkpoint pass-or-absent). Stdout JSON is the machine contract; stderr may still show the first unsigned `exit 3` line if a later structural break wins. Checkpoint `match` and `extends` stay exit **0** (`ok: true`); `diverges` / `exactMismatch` / `exportSigInvalid` stay exit **1**. This reports; it does not repair historical chain breaks.
*   **Chain Integrity**: Each committed ledger entry carries a `prev_hash` linking it to the prior chain head, and a signed `chain_head` row binds the latest entry hash, genesis boundary, and chain length. `ledgerful verify --signatures --chain` validates the chain end-to-end and reports the exact break location. The chain verifies the integrity and continuity of the presented chain; detection of rollback to an earlier valid state requires an independently retained chain head. **Operator path:** `ledgerful export head` writes a thin `chain_head.json` checkpoint (or `--stdout` / `-o -` for pure pretty JSON); `ledgerful verify --signatures --against-export <path>` compares against a SOC2 zip **or** bare JSON with **checkpoint (extends-or-equals)** semantics by default (`--exact` restores full snapshot equality). See `docs/chain-checkpoint.md`. This is not local immutability.
...
*   **Chain head checkpoint export**: `ledgerful export head [--out <path>] [--force] [--stdout]` writes the live chain head as bare JSON for periodic off-machine retention (default `./ledgerful-chain-head.json`). Use `--stdout` or `-o -` for pure pretty JSON on stdout (no SUCCESS banner; does not create a file named `-`). File-mode SUCCESS is a checkpoint write, not a verification (`verify --signatures --against-export`). Refuses unsigned heads when `intent.require_signing` is true; when `require_signing` is false may write an unsigned head with a warning. File mode keeps the same path-safety as evidence export. See `docs/chain-checkpoint.md`.
...
*   **Config provenance (0323 / 0342 / 0362):** `config verify` attributes file/env/default via `origin`/`location` (no secret values). Human unscoped names the four-section health catalog (Backend, Semantic, Ask, Gate) and clap next (`--verbose`, `config view --section`). Config-backed semantic concurrency rows emit origin; Auto/derived rows omit it; Inherited keeps `source: inherited` with file origin. Empty/whitespace TOML `base_url` origin agrees with `resolve_string` (no trim): whitespace is file, empty after env/dotenv miss is default and is omitted without `--verbose`. `config schema` names the source file and distinguishes declared-optional from unknown requiredness. Empty `.env.example` `KEY=` stays not required (0216). Human `config diff` prints reference paths. `config view` stays a redacted Config dump; unscoped human Next names `--section`/`--key` and `config verify --verbose`.
*   **Scoped Test Selection**: `verify --scope fast` uses the `test_mapping` index to run only the tests covering changed files via nextest filtersets. Quoted `-E '…'` and `-E "…"` filtersets (`test()` / `+`) prepare as Direct argv; unquoted `\` skips the next character (POSIX); unescaped unquoted metas still refuse. When mapping cannot scope, it **refuses** (exit ≠ 0) rather than silently running the full suite; shared infrastructure still falls back to full; operators can opt in with `--allow-full-fallback`. The pre-push hook uses `--scope fast` (~33s vs ~6m20s for full).
...
*   **CLI step timeout**: `verify --timeout` is optional. Omitting it leaves each step's planned budget, including `[verify] suite_timeout_secs` on auto-plan `cargo test` / `cargo nextest` steps (built-in 400 when that key is absent). An explicit value is `min(planned, flag)`. Auto `--timeout 0` stays the 400-second ceiling. Fast empty-changes stays fmt 60 / clippy 400; the normal auto plan still stamps fmt at 400. A step that hits the cap is a JSON fail row (`exitCode` 124, `Step timed out after`) and later steps are not started. Manual command and `[[verify.steps]]` keep their own timeouts (omitted manual is 600 seconds; manual `0` is an immediate timeout row, not a disabled clock). Signature verify (`--signatures` / `--chain` / `--against-export`) ignores the flag. Full scope is not a 40-second guarantee.
...
*   **Semantic Context Assembly**: Budget-aware assembly of structural, semantic, and historical context for prompts. Vector snippets are function-grain with `fn name` / `Type.method` in the embed text (declaration-only `mod x;` is skipped). `search --semantic` and `ask --semantic` resolve embedding dimensions from config, probe, or the stored index (never a 384 guess); Ask falls back to Tantivy when vectors cannot run. `index --semantic` recreates the stored width at the preferred dimension (destructive wipe + hash purge); query paths do not. Default **global** `ask` skips oversized gather for LLM-instruction / ping tokens; `--semantic` and one-word subsystem queries (`config`, `MCP`) still gather. KG neighborhood is not attached on mere clean-tree `is_global`. An unconfigured Ask backend on the skip path is a configure error (not `Note: no retrieved snippets`). Ask RRF counts a path once per list (first rank); `search_term_exact` locates full CamelCase / PascalCase identifiers after dual-emit. FTS stamp `code_tokenizer_v3` forces a one-time rebuild of existing v2 postings (`search` / `index`, not Ask).
*   **Modes of Assistance**:
...
* **Dead Code Pruning**: `dead-code --prune` interactively removes high-confidence dead-code *candidates* (heuristic evidence, not proof) with `inquire` prompts, wrapped in a pending ledger transaction for verifiable safety. Unresolved trait-impl dispatch (Ambiguous/unresolved method-call edges) is not scored as unreachable.
* **Nightly Scheduler**: `schedule setup-nightly` installs a cross-platform nightly task (Windows schtasks / Unix crontab) that runs `git fetch` + `index --analyze-graph` (no `--auto-scip` by default) to keep the search/observability cache warm without workday file-lock contention. Opt-in only — not installed by bare `init`. `init --operator-pack` installs the task if it is missing.
* **Operator pack (0403)**: opt-in `init --operator-pack` fills product-default coverage/bridge keys and installs nightly only if missing. Bare `init` stays default-off (0186-A). Does not write Ask providers or run SCIP.
