...
*   **Chain head checkpoint export**: `ledgerful export head [--out <path>] [--force]` writes the live signed `chain_head` as bare JSON for periodic off-machine retention (default `./ledgerful-chain-head.json`). Refuses unsigned heads when `intent.require_signing` is true. Same path-safety as evidence export. See `docs/chain-checkpoint.md`.
