...
*   **Scoped Test Selection**: `verify --scope fast` uses the `test_mapping` index to run only the tests covering changed files via nextest filtersets. Quoted `-E '…'` and `-E "…"` filtersets (`test()` / `+`) prepare as Direct argv; unquoted shell metacharacters still refuse. When mapping cannot scope, it **refuses** (exit ≠ 0) rather than silently running the full suite; shared infrastructure still falls back to full; operators can opt in with `--allow-full-fallback`. The pre-push hook uses `--scope fast` (~33s vs ~6m20s for full).
