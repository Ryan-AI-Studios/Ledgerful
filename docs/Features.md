...
*   **Chain head checkpoint export**: `ledgerful export head [--out <path>] [--force] [--stdout]` writes the live chain head as bare JSON for periodic off-machine retention (default `./ledgerful-chain-head.json`). Use `--stdout` or `-o -` for pure pretty JSON on stdout (no SUCCESS banner; does not create a file named `-`). Refuses unsigned heads when `intent.require_signing` is true; when `require_signing` is false may write an unsigned head with a warning. File mode keeps the same path-safety as evidence export. See `docs/chain-checkpoint.md`.
