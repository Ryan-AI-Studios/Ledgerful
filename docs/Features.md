...
*   **Control-Scoped Evidence Export**: `ledgerful export evidence --profile soc2 --control CC8.1 --control "CC7.*"` appends an additive control lens (`control-lens/cover.md` + `index.json`) to the signed bundle without changing it. Exact IDs and family wildcards are supported; unknown selectors are rejected. See `docs/mappings/soc2.md`.
