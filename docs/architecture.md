...
- `web` starts the optional local dashboard (feature-gated behind `web`).
  ...
  hidden `--token` for operators). Default is not to print the token: it is written
  to `.ledgerful/web-session-token` and only the path is printed (`--print-token=true`
  is an opt-in escape hatch). Prefer env or the token file over shell history.
  `web start --open` mints a single-use handoff code, opens `{base}#c=…`, and the SPA
  exchanges it via `POST /api/session/exchange` for the bearer token (never in a URL).
  Empty/short explicit tokens refuse start. `--spa-dir` is canonicalized and contained
  (allow-root / dashboard marker). `--allow-public` requires `LEDGERFUL_WEB_PEER_ALLOWLIST`.
